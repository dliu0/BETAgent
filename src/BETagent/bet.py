...
class DDMbet:
    ...
    def plot_cross(self, dataX, dataY, cross):
        ...
        plt.tight_layout()
        # Return the figure so callers can save or show as desired
        return fig
    ...
    def runBET(self, dataX, dataY, plot=False, find_min = False, max_depth=8, p_value_threshold=0.05, plot_save_path: str = None, show_plot: bool = True):
        """
        ...
        if plot:
            fig = self.plot_cross(dataX, dataY, min_cross)
            # If a save path is provided, save the figure and close it
            if plot_save_path:
                try:
